...
class IBVSTransformerTraining():
    def __init__(self, args, use_loc_tok=True, use_cls_tok=True, flattened_tensor=True, loss_func=nn.SmoothL1Loss(), beta=1, use_depth=False, output_features=[6]):
        ...
        self.vit = IBVSTransformer(args=args, in_chans=in_chans, use_loc_tok=use_loc_tok, use_cls_tok=use_cls_tok, output_features=output_features, act_layer=act_layer)
    ...
    def get_errors(self, current_vel, target_vel):
        current_vel = current_vel.clone().cpu().detach().numpy()
        target_vel = target_vel.clone().cpu().detach().numpy()
        t_error = np.mean(np.linalg.norm(target_vel[:,:3]-current_vel[:,:3],axis=-1), axis=0)
        t_scale_error = np.mean(target_vel[:,3]-current_vel[:,3], axis=0)
        r_error = np.mean(np.linalg.norm(target_vel[:,4:7]-current_vel[:,4:7],axis=-1), axis=0)
        r_scale_error = np.mean(target_vel[:,7]-current_vel[:,7], axis=0)
        return t_error, t_scale_error, r_error, r_scale_error
    ...
    def train_transformer(self, args, test_case, it, pretrained=True, 
        ...
        image_path_loader = ImagePathLoader(from_harddive=True, without_scaling_factors=without_scale, blender_path=args.blender_data)
        ...
        param_str += '_' + str(self.weight_decay)
        param_str += '_' + str(args.drop_out)
        if normalize_imgs:
            param_str += '_norm'
        ...
        for epoch in range(self.EPOCHS):
            ...
            with tqdm(train_loader) as tr_loader:
                for i, batch in enumerate(tr_loader, 0):
                    ...
                    r_error_batches.append(r_err)
                    r_scale_error_batches.append(r_s_err)
            ...
            # Error values
            train_t_err = np.mean(t_error_batches)
            t_error_batches.clear()
            train_t_s_err = np.mean(t_scale_error_batches)
            t_scale_error_batches.clear()
            train_r_err = np.mean(r_error_batches)
            r_error_batches.clear()
            train_r_s_err = np.mean(r_scale_error_batches)
            r_scale_error_batches.clear()
            # Accuracy and loss
            accuracies_train = np.mean(train_accuracies_batches)
            train_accuracies.append(accuracies_train)
            train_losses.append(loss_train)
            train_string = train_string + f"Epoch {epoch + 1}/{self.EPOCHS}, time: {delta}, loss: {loss_train}, loss_type: {self.loss_func.__class__.__name__}, accuracy: {accuracies_train}\n"
            train_string += f"t_err: {train_t_err}, t_s_err: {train_t_s_err}, r_err: {train_r_err}, r_s_err: {train_r_s_err}\n\n"
            ...
            with torch.no_grad():
                ...
                with tqdm(test_loader) as ts_loader:
                    for i, batch in enumerate(ts_loader, 0):
                        ts_loader.set_description(f"Test Epoch {epoch + 1}")
                        query_image, target_image, vel_vec = batch
                        vel_vec = vel_vec.view(vel_vec.shape[0], 1, self.learn_vec_len)
                        # query_image, target_image, vel_vec = query_image.to(self.device), target_image.to(self.device), vel_vec.to(self.device)
                        v_normalized = self.vit(goal=target_image,
                        ...
                        ts_loader.set_postfix(loss=loss.item(), accuracy=test_accuracy, lr=current_lr)
                loss_test = np.mean(test_loss_batches)
                test_losses.append(loss_test)
                test_accuracy = np.mean(test_accuracies_batches)
                test_accuracies.append(test_accuracy)
                # Error values
                test_t_err = np.mean(t_error_batches)
                del t_error_batches
                test_t_s_err = np.mean(t_scale_error_batches)
                del t_scale_error_batches
                test_r_err = np.mean(r_error_batches)
                del r_error_batches
                test_r_s_err = np.mean(r_scale_error_batches)
                del r_scale_error_batches

                if test:
                    test_string += 'Test'
                test_string = test_string + f"Epoch {epoch + 1}/{self.EPOCHS}, loss: {test_loss}, accuracy: {test_accuracy}\n"
                test_string += f"t_err: {test_t_err}, t_s_err: {test_t_s_err}, r_err: {test_r_err}, r_s_err: {test_r_s_err}\n\n"
